...
const alertRegister = document.getElementById(
  "alert-register",
) as HTMLDivElement;
  ...
  formLogin.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        mostrarAlerta(
          alertLogin,
          "Acceso concedido. Entrando al sistema...",
          "success",
        );
        ...
        if (!tokenParaGuardar || !usuarioParaGuardar) {
          throw new Error(
            "El servidor no devolvió el token o los datos del usuario correctamente.",
          );
        }

        sessionStorage.setItem("token_jurassic", tokenParaGuardar);
      ...
      mostrarAlerta(
        alertLogin,
        "Error de conexión con el servidor principal o datos corruptos.",
        "danger",
      );
    }
  });
...
if (formRegister) {
  formRegister.addEventListener("submit", async (e) => {
    ...
    const nick = (document.getElementById("reg-nick") as HTMLInputElement)
      .value;
    ...
    const password_confirmation = (
      document.getElementById("reg-password-conf") as HTMLInputElement
    ).value;
    const foto = (document.getElementById("reg-foto") as HTMLInputElement)
      .files?.[0];
