    ...
    try {
        ...
        for (let f = 1; f <= maxFila; f++) {
            ...
            for (let col = 1; col <= maxColumna; col++) {
                ...
                if (celda) {
                    const clasesBorde = coloresSeguiridad[celda.nivel_seguridad] || 'border-secondary';
                    const badgeClase  = badgeSeguiridad[celda.nivel_seguridad] || 'bg-secondary';

                    const alimentoColor = celda.alimento_porcentaje > 50 ? 'bg-success'
                        : celda.alimento_porcentaje > 25 ? 'bg-warning'
                        : 'bg-danger';
...
const inputColumna      = document.getElementById('celda-columna') as HTMLInputElement;
const selectSeguridad   = document.getElementById('celda-seguridad') as HTMLSelectElement;
const inputAlimento     = document.getElementById('celda-alimento') as HTMLInputElement;
const inputAverias      = document.getElementById('celda-averias') as HTMLInputElement;
...
const abrirModalEditar = (id: number, fila: number, columna: number, seguridad: string, alimento: number, averias: number) => {
...
document.getElementById('btn-nueva-celda')?.addEventListener('click', abrirModalCrear);

// ─── GUARDAR CELDA (crear o editar) ───────────────────────────────────────────
document.getElementById('btn-guardar-celda')?.addEventListener('click', async () => {
...
const modalConfirmarBorradoEl = document.getElementById('modalConfirmarBorrado')!;
...
document.getElementById('btn-confirmar-borrado')?.addEventListener('click', async () => {
    ...
    try {
        const res = await fetch(`${CONSTANTS.API.BASE_URL}${CONSTANTS.API.CELDAS}/${celdaIdAEliminar}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Accept': 'application/json',
